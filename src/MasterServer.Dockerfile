...
COPY ./conf/druid/master/_common/common.runtime.properties /druid/conf/druid/_common/common.runtime.properties
COPY ./conf/druid/single/jvm.config /druid/conf/druid/single/jvm.config

RUN rm -rf *.tar.gz

COPY ./script/masterserver/start-single.sh /druid/start-single.sh
